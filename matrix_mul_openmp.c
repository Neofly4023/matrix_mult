    ...
    for (int i = 0; i < height; i += tile_size) {
        for (int j = 0; j < width; j += tile_size) {
            for (int k = 0; k < width; k += tile_size){
                for (int y = i; y < i + tile_size; y++) { // errors started from here since the condition for ceiling y is nbr_of_block*index + deplcaement
                    for (int x = j; x < j + tile_size; x++) {
                        for (int z = k; z < k + tile_size; z++) {
